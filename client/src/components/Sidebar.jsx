...
const Sidebar = () => {
            ...
            {navItems.map((item)=>{
                ...
                return (
                    <Link key={item.name} to={item.href} className={
                        `group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300": "text-slate-300 hover:text-white hover:bg-white/4" 
                        }`
                    }>
                        {
  ...
  return (
    ...
    {/* sidebar desktop */}
    <aside className="hidden lg:flex flex-col h-full w-[260px] bg-gradient-to-b from-slate-900 to-slate-950 text-white shrink-0 border-r border-white/4">
        {sidebarContent}
    </aside>
