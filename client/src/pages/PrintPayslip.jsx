    ...
    <div className="max-w-2xl mx-auto p-8 bg-white animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div>
          ...
          <p className="text-slate-900 font-semibold ">{payslip.employee?.firstName} {payslip.employee?.lastName}</p>
        </div>
        <div>
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Position</p>
          <p className="text-slate-900 font-semibold ">{payslip.employee?.position}</p>
        </div>
        <div>
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Email</p>
          <p className="text-slate-900 font-semibold ">{payslip.employee?.email}</p>
        </div>
        <div>
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Period</p>
          <p className="text-slate-900 font-semibold ">{format(new Date(payslip.year, payslip.month-1),"MMMM yyyy")}</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            <tr className="border-t-2 border-slate-200 bg-slate-50">
              ...
              <td className="py-4 px-4 text-right text-slate-900 text-lg font-bold">${payslip.netSalary?.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
